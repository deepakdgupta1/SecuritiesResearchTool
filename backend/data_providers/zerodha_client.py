...
class ZerodhaProvider(BaseDataProvider):
    ...
    def __init__(self, rate_limit: float = ZERODHA_RATE_LIMIT):
        ...
        try:
            ...
            self.logger.info(f"Initialized ZerodhaProvider with rate_limit={rate_limit}/s")
            
        except Exception as e:
            raise DataProviderError(f"Failed to initialize Zerodha provider: {e}")
    ...
    def get_historical_data(
        ...
        **kwargs
        ...
        try:
            ...
            if token is None:
                raise DataNotFoundError(f"Symbol {symbol} not found on {exchange}")
            
            self.logger.debug(f"Fetching {symbol} (token:{token}) from {start_date} to {end_date}")
