...
def check_data_freshness(session) -> Dict:
    ...
    for market, last_date, count in results:
        ...
        logger.info(f"Market {market}: Last date {
                    last_date} ({count} symbols)")
...
def check_price_anomalies(session) -> List[Dict]:
    ...
    if bad_prices:
        logger.warning(
            f"Found {
                len(bad_prices)} records with zero/negative prices")
        for p in bad_prices:
            anomalies.append({
                'type': 'BAD_PRICE',
...
def check_missing_data(session) -> List[Dict]:
    ...
    missing = []
    # Allow 5 days lag (weekends/holidays)
        ...
        with get_session() as session:
            ...
            if not anomalies and not missing:
                ...
                if missing:
                    logger.warning(
                        f"⚠ Found {
                            len(missing)} symbols with stale data")
