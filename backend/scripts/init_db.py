...
from backend.core.database import engine, check_database_connection
...
def create_timescaledb_extension() -> bool:
    ...
    try:
        ...
        logger.warning("Continuing without TimescaleDB - performance may be degraded")
        return False
...
def convert_to_hypertable() -> bool:
    ...
    try:
        with engine.connect() as conn:
            ...
            # Note: Using symbol_id as partitioning column as well for better performance
            conn.execute(
                text(
        ...
        logger.warning("Table created as regular table - consider manual conversion")
        return False
...
def main() -> int:
    ...
    if timescaledb_available:
        ...
        if not convert_to_hypertable():
            logger.warning("⚠ Hypertable conversion failed - continuing anyway")
        logger.info("")
    else:
        logger.info("Step 4: Skipping hypertable conversion (TimescaleDB not available)")
        logger.info("")
    
    # Step 5: Create additional indexes
    logger.info("Step 5: Creating additional indexes...")
    if not create_indexes():
        logger.warning("⚠ Additional index creation failed - continuing anyway")
    logger.info("")
