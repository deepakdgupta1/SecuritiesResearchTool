...
class Symbol(Base):
    ...
    pattern_detections = relationship(
        ...
        back_populates="symbol",
        cascade="all, delete-orphan")
    trade_recommendations = relationship(
        "TradeRecommendation",
        back_populates="symbol",
        cascade="all, delete-orphan")
...
class DerivedMetrics(Base):
    ...
    def __repr__(self) -> str:
        return f"<DerivedMetrics(symbol_id={
            self.symbol_id}, date={
            self.date})>"
...
class PatternDetection(Base):
    ...
    # VCP, CUP_HANDLE, TREND_TEMPLATE, etc.
    ...
    trade_recommendations = relationship(
        "TradeRecommendation",
        back_populates="pattern_detection",
        cascade="all, delete-orphan")

    # Additional indexes
    __table_args__ = (
        Index(
            "ix_pattern_detections_date_symbol",
            "detection_date",
            "symbol_id"),
        Index(
            "ix_pattern_detections_type_stage",
            "pattern_type",
            "weinstein_stage"),
    )

    def __repr__(self) -> str:
        return f"<PatternDetection(id={
            self.id}, symbol_id={
            self.symbol_id}, pattern='{
            self.pattern_type}')>"
...
class TradeRecommendation(Base):
    ...
    # References
    pattern_detection_id = Column(
        Integer,
        ForeignKey("pattern_detections.id"),
        index=True)
    symbol_id = Column(
        Integer,
        ForeignKey("symbols.id"),
    ...
    status = Column(
        String(20),
        default="OPEN",
        index=True)  # OPEN, CLOSED, CANCELLED

    # Audit
    created_at = Column(DateTime, default=datetime.utcnow)

    # Relationships
    pattern_detection = relationship(
        "PatternDetection",
        back_populates="trade_recommendations")
    symbol = relationship("Symbol", back_populates="trade_recommendations")

    # Additional indexes
    __table_args__ = (
        Index(
            "ix_trade_recommendations_date_status",
            "recommendation_date",
            "status"),
    )

    def __repr__(self) -> str:
        return f"<TradeRecommendation(id={
            self.id}, symbol_id={
            self.symbol_id}, type='{
            self.trade_type}')>"
...
class BacktestResult(Base):
    ...
    def __repr__(self) -> str:
        return f"<BacktestResult(id={
            self.id}, name='{
            self.backtest_name}', return={
            self.total_return})>"
...
class BacktestTrade(Base):
    ...
    # STOP_LOSS, TAKE_PROFIT, TRAILING_STOP, SIGNAL
    exit_reason = Column(String(50))

    # Relationships
    backtest_result = relationship(
        "BacktestResult",
        back_populates="backtest_trades")
