...
class TestSessionManagement:
    ...
    def test_get_session_commits_on_success(self):
        ...
        with get_session() as session:
            result = session.query(Symbol).filter_by(symbol="TEST_SESSION").first()
            if result:
                # Clean up
    ...
    def test_get_session_rolls_back_on_error(self):
        ...
        with get_session() as session:
            result = session.query(Symbol).filter_by(symbol="TEST_ROLLBACK").first()
...
class TestConnectionPooling:
    ...
    def test_session_isolation(self):
        ...
        try:
            ...
            result = session2.query(Symbol).filter_by(symbol="TEST_ISOLATION").first()
